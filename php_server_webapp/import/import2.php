...
<?php
// インポートしたいテーブルにあわせて、追加データテーブルと元データテーブルが必要となる。
// ex, インポートしたいテーブル:「検査」とすると「検査追加データ」、「検査元データ」テーブルを用意
// 追加,元データテーブルはインポートしたいテーブルをコピーして作る。
// 追加,元データテーブルは主キーを削除しておくこと DROP INDEX `PRIMARY` ON TABLENAME;
// 追加,元データテーブルのIDはデフォルト
// NULLに設定しておくこと
...
//      echo ("</pre>");
    ...
    try{
      ...
      $stmt->execute();
      // SQL 追加データテーブルデータ全削除 
      $query="TRUNCATE ".$tablename."追加データ ";
      $stmt = $dbh->query($query);
      $stmt->execute();
      // SQL INSERT
      foreach ( $csvdata as $csvkey => $csvvalue ){
...
//------------------------------------------------------------------------------------ 
      echo ("<H2>取込確認</H2>"); // インポートストアドプロシジャ発行し、追加データテーブルを表示
      // インポートストアドプロシジャを介するのはデータ加工を適宜行う必要があるため
      $query="CALL ".$tablename."インポート();";
      $query=rtrim($query,',').");";
      $stmt = $dbh->query($query);
      $query="UPDATE ".$tablename."追加データ SET ".$tablename."ID = NULL;";
      $stmt = $dbh->query($query);
      // $stmt->execute();
