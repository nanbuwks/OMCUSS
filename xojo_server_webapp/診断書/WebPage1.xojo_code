...
Begin WebPage WebPage1
   ...
   Begin WebButton writeButton2
      ...
      _VerticalPercent=   0.0
   End
   Begin WebListBox ListBox診断書
      AlternateRowColor=   &cEDF3FE00
      ColumnCount     =   5
      ColumnWidths    =   "0%,15%,15%,40%,30%"
      Cursor          =   0
      Enabled         =   True
      HasHeading      =   True
      HeaderStyle     =   "0"
      ...
      HorizontalCenter=   0
      Index           =   -2147483648
      InitialValue    =   "診断書ID	診断書日	診断医師	内容	メモ			"
      Left            =   55
      ListIndex       =   -1
      LockBottom      =   False
      LockedInPosition=   False
   ...
   Begin WebListBox ListBoxXP検査
      ...
      HorizontalCenter=   0
      Index           =   -2147483648
      InitialValue    =   "年月日	XP検査ID	フィルムNO	判定	判読"
      Left            =   729
      ListIndex       =   -1
      LockBottom      =   False
      LockedInPosition=   False
...
#tag WindowCode
	...
	#tag Method, Flags = &h21
		Private Sub setlink()
		  ...
		  var Host(),診断書ID as String
		  
		  If ListBox診断書.ListIndex < 0 Then
		    MessageBox("リストから選択してください")
		  Else
		    
		    診断書ID = ListBox診断書.Cell(ListBox診断書.ListIndex, 0)
		    // var 日本語英語 as integer
		    // 日本語英語 = writeSyomeiRadio.SelectedCell.Left
		    
		    Host = Session.Header("Host").split(":")
		    // if 0 = 日本語英語 then
		    Link1.URL="http://"+Host(0).DefineEncoding(Encodings.UTF8).ToText+"/template/mkpdf.php?use=診断書&key="+診断書ID
		    // else
	...
	#tag Method, Flags = &h0
		Sub 取得診断書()
		  ...
		  try
		    sql = "SELECT 診断書ID,DATE_FORMAT(診断書日, '%Y/%m/%d') AS 診断書日,診断医師,内容,メモ"_
		    +" FROM 診断書 where 学生番号 = '"+id+"' order by 診断書日 desc"
		    rs =app.db.SelectSQL(sql)
		    
		  Catch e as DatabaseException
		    messagebox e.Message
		  end try
		  WebPage1.Listbox診断書.DeleteAllRows
		  
		  try
		    for each row as databaserow in rs
		      
		      WebPage1.Listbox診断書.AddRow(  row.column("診断書ID").StringValue,_
		      row.column("診断書日").StringValue,_
		      row.column("診断医師").StringValue,_
		      replaceall(row.column("内容").StringValue,String.ChrByte(10),Encodings.UTF8.Chr(&h23CE)),_
		      replaceall(row.column("メモ").StringValue,String.ChrByte(10),Encodings.UTF8.Chr(&h23CE)))
	...
	#tag Constant, Name = Untitled3, Type = , Dynamic = False, Default = \"", Scope = Public
	#tag EndConstant

	#tag Constant, Name = Untitled4, Type = , Dynamic = False, Default = \"", Scope = Public
	#tag EndConstant

	#tag Constant, Name = Untitled5, Type = , Dynamic = False, Default = \"", Scope = Public
	#tag EndConstant
...
#tag Events writeButton1
	#tag Event
		Sub Action()
		  ...
		  sql=_
		  "INSERT INTO 診断書 (学生番号,診断書日,診断医師,内容,メモ) VALUES "_
		  +"('"_
		  +writeField.text+"',"_
		  +"curdate(),'"_
	...
	#tag Event
		Sub Action()
		  ...
		  Else
		    ...
		    write内容Area.text= replaceall(ListBox診断書.Cell(ListBox診断書.ListIndex, 3).DefineEncoding(Encodings.UTF8),Encodings.UTF8.Chr(&h23CE),String.ChrByte(10))
		    writeメモArea2.text= replaceall(ListBox診断書.Cell(ListBox診断書.ListIndex, 4).DefineEncoding(Encodings.UTF8),Encodings.UTF8.Chr(&h23CE),String.ChrByte(10))
	...
	#tag Event
		Sub SelectionChanged()
		  ...
		  Else
		    ...
		    View内容Area.text= replaceall(Me.Cell(Me.ListIndex, 3).DefineEncoding(Encodings.UTF8),Encodings.UTF8.Chr(&h23CE),String.ChrByte(10))
		    writeメモArea1.text= replaceall(Me.Cell(Me.ListIndex, 4).DefineEncoding(Encodings.UTF8),Encodings.UTF8.Chr(&h23CE),String.ChrByte(10))
		    setlink()
		  End If
	...
	#tag Event
		Sub Action()
		  ...
		  Else
		    ...
		    "UPDATE 診断書 SET メモ=" _
		    +"'" _
		    +replaceall(writeメモArea1.text,String.ChrByte(10),"\n") _
		    +"' WHERE 診断書ID='" _
		    +診断書ID _
		    +"';"
		    try
