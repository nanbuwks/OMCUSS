...
Begin WebPage WebPage1
   ...
   Begin WebLabel Label13
      ...
      _VerticalPercent=   0.0
   End
   Begin WebTextField write病院名Field
      AutoCapitalize  =   True
      AutoComplete    =   True
      AutoCorrect     =   True
      CueText         =   ""
   ...
   Begin WebListBox ListBox紹介状
      AlternateRowColor=   &cEDF3FE00
      ColumnCount     =   8
      ColumnWidths    =   "0%,11%,11%,11%,11%,11%,30%,15%"
      Cursor          =   0
      Enabled         =   True
      HasHeading      =   True
      HeaderStyle     =   "0"
      ...
      HorizontalCenter=   0
      Index           =   -2147483648
      InitialValue    =   "紹介状ID	紹介状日	紹介医師	病院名	受診科	医師氏名	内容	メモ"
      Left            =   55
      ListIndex       =   -1
      LockBottom      =   False
      LockedInPosition=   False
...
#tag WindowCode
	...
	#tag Method, Flags = &h21
		Private Sub setlink()
		  ...
		  Else
		    ...
		    紹介状ID = ListBox紹介状.Cell(ListBox紹介状.ListIndex, 0)
	...
	#tag Method, Flags = &h0
		Sub 取得紹介状()
		  ...
		  try
		    sql = "SELECT 紹介状ID,DATE_FORMAT(紹介状日, '%Y/%m/%d') AS 紹介状日,紹介医師,病院名,受診科,医師氏名,内容,メモ"_
		  ...
		  WebPage1.Listbox紹介状.DeleteAllRows
		  
		  try
		    for each row as databaserow in rs
		      
		      WebPage1.Listbox紹介状.AddRow(  row.column("紹介状ID").StringValue,_
		      row.column("紹介状日").StringValue,_
		      row.column("紹介医師").StringValue,_
		      row.column("病院名").StringValue,_
		      row.column("受診科").StringValue,_
		      row.column("医師氏名").StringValue,_
