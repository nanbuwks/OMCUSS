...
Begin WebPage WebPage1
   ...
   Begin WebLabel Label13
      ...
      _VerticalPercent=   0.0
   End
   Begin WebTextField write総合判定Field
      AutoCapitalize  =   True
      AutoComplete    =   True
      AutoCorrect     =   True
      CueText         =   ""
      ...
      _VerticalPercent=   0.0
   End
   Begin WebListBox ListBox総合判定候補
      AlternateRowColor=   &cEDF3FE00
      ColumnCount     =   1
      ColumnWidths    =   "*"
      Cursor          =   0
   ...
   Begin WebListBox ListBox総合判定
      ...
      HorizontalCenter=   0
      Index           =   -2147483648
      InitialValue    =   "判定日	医師所見	指導区分	impression"
      Left            =   877
      ListIndex       =   -1
      LockBottom      =   False
      LockedInPosition=   False
   ...
   Begin WebLabel Label17
      ...
      _VerticalPercent=   0.0
   End
   Begin WebListBox ListBox指導区分候補
      AlternateRowColor=   &cEDF3FE00
      ColumnCount     =   2
      ColumnWidths    =   "20%,80%"
      Cursor          =   0
...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Sub フィールドクリア()
		  ...
		  write総合判定Field.text=""
		  if -1 < ListBox総合判定候補.ListIndex then ListBox総合判定候補.Selected( ListBox総合判定候補.ListIndex ) = False
		  if -1 < ListBox指導区分候補.ListIndex then ListBox指導区分候補.Selected( ListBox指導区分候補.ListIndex ) = False
	...
	#tag Method, Flags = &h0
		Sub 取得候補医師()
		  ...
		  try
		    sql = "SELECT 診察医師名 FROM `医師受診` WHERE DATE_FORMAT(医師受診日 - INTERVAL 2 MONTH, '%Y') LIKE "+writeNendoField.Text+"  GROUP BY 診察医師名 ;"
		    // ３月から新年度
		    rs =app.db.SelectSQL(sql)
	...
	#tag Method, Flags = &h0
		Sub 取得総合判定()
		  ...
		  try
		    sql = "SELECT 総合判定ID,DATE_FORMAT(総合判定日, '%Y/%m/%d') AS 総合判定日,年度(総合判定日,"+writeNendoField.text+") AS 年度,医師所見,指導区分.指導区分 AS 指導区分,impression"_
...
#tag Events ListBox医師候補
	...
	#tag EndEvent
#tag EndEvents
#tag Events write総合判定Field
	#tag Event
		Sub TextChanged()
		  If 0 < write総合判定Field.Text.Length and -1 < ListBox指導区分候補.ListIndex Then 
...
#tag Events writeButton1
	#tag Event
		Sub Action()
		  ...
		  sql=_
		  "INSERT INTO 総合判定 (学生番号,総合判定日,医師所見,指導区分) VALUES "_
		  +"('"_
		  +writeField.text+"',"_
		  +"curdate(),'"_
		  +write総合判定Field.text+"',"_
		  +ListBox指導区分候補.Cell(ListBox指導区分候補.ListIndex,0)+");"
		  try
		    rs =app.db.selectSQL(sql)
		  Catch e as DatabaseException
		    messagebox e.Message
	...
	#tag Event
		Sub SelectionChanged()
		  ...
		  Else
		    ...
		    write総合判定Field.text=content
		  End If
		  
		End Sub
	#tag EndEvent
#tag EndEvents
#tag Events ListBox指導区分候補
	#tag Event
		Sub Shown()
		  // 取得診察所見リスト()
		End Sub
