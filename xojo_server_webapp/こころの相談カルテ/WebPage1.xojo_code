...
#tag WindowCode
	...
	#tag Method, Flags = &h21
		Private Sub setlink()
		  ...
		  // if 0 = 日本語英語 then
		  Link1.URL="http://"+Host(0).DefineEncoding(Encodings.UTF8).ToText+"/template/mkpdf.php?use=こころの相談カルテ&key="+writeField.text
		  // else
		  // Link1.URL="http://"+Host(0).DefineEncoding(Encodings.UTF8).ToText+"/template/mkpdf.php?use=こころの相談カルテ&key="+writeField.text
		  // end if
		...
		Sub 取得個人()
		  ...
		  try
		    for each row as databaserow in rs
		      ...
		      Listbox個人検索.AddRow(  row.column("学生番号").StringValue,row.column("フリガナ").StringValue,row.column("漢字氏名").StringValue,row.column("生年月日").StringValue,row.column("性別").StringValue,row.column("年次").StringValue,row.column("学部").StringValue,row.column("学科").StringValue)
